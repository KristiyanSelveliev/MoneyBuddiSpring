...
public class ExpenseController {
	...
	public String showExpenseCategories(HttpSession session, HttpServletRequest request) throws Exception {
		...
		List<Category> categories = categoryDao.getAllCategoriesByUserAndType(userDao.getUserById(userId),
	...
	public String addExpense(HttpSession session, HttpServletRequest request) {
		try {
			...
			if (amountAsString != null && !amountAsString.isEmpty()) {
				...
				// create transaction (needed: amount, currency, acount, date, category,
				// type=income)
